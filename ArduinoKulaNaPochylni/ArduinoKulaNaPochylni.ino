// ------------ kod do sterowania pochylnia ------------------ // #include <Wire.h> // #include <Servo.h> // unsigned long myTime;
 // Servo myservo;
 // float distance;
 // float kp = 3.0;
 //potegowanie bledu 3.0 // float ki = 0.5;
 //potegowanie bledu w czasie, zapis historyczny, jesli zwiekszamy, dluzej bedzie pamietal co sie dzielo wczesniej, jak cyfry wyskoczyly to dlugo czekal, male 0.5 // float kd = 0.8;
 //poteguje roznice pomiedzy ostatnim a obecnym bledem, jak szybko ma zareagowac (przewiduje troche przyszlosc) kd jest 2x mniejsze lub wieksze niz kp (mniejsze wg macmaca) 0.8 // float integral = 0.0;
 // float derivative = 0.0;
 // float previousError = 0.0;
 // float distance_point = 18.0;
 // int servo_zero = 0;
 // int t = 100;
 // float get_dist(int n){ // long sum=0;
 // for(int i=0;i<n;i++){ // sum=sum+analogRead(A0);
 // }

 // float adc=sum/n;
 // //zbiera wartosc 100 razy i usrednienie // float distance_cm = 17569.7 * pow(adc, -1.2062);
 //przeliczenie na cm // return(distance_cm);
 // //sciaga odleglosc od czurnika // }

 // void PID(){ // //to jest algorytm caly praktycznie, // float proportional = distance-distance_point;
 //proporcjonalny, im wiekszy blad tym silnejsze dzialanie regulatora, tutaj jest szybkosc, troche ponad 1 // integral = integral+proportional*0.1;
 // calkujacy, 0,1 jest do normalizacji wartosci, integral jest globalne pamietane z poprzedniej iteracji // derivative=(proportional-previousError)/0.1;
 //rozniczkujacy, roznica pomiedzy aktualnym bleden i poprzedniej iteracji, mowi w która strone powinnismy się krecic // float output=kp*proportional+ki*integral+kd*derivative;
 // //kp ki kd to sa parametry do regulowania pida, tym manipulujemy // //distance point -> punkt docelowy // //wypisanie wartosci // Serial.print(distance);
 // Serial.print(" : ");
 // Serial.print(proportional);
//blad pomiedzy tym co ma a tym co powinno byc // Serial.print(" : ");
 // Serial.println(output);
 //wysjcie z pida, powinien być w zakresie –10/10 // previousError=proportional;
 // myservo.write(servo_zero+output);
 //tu sterujemy serwem pochylnia // // servo_zero jest rownym poziomem, sprawdzic i ustawic to samodzielnie // }
...
//aktualny czas systemu // }
 ...
 // ------------- ustawienia bazowe -------------- #include <Wire.h> #include <Servo.h> unsigned long myTime;
 Servo myservo;
 float distance;
 ...
 float integral = 0.0;//todo trzeba ustawic integral
 //integral -> i = i+error
 // i = 100+0 jesli integral ustawimy > 0 to on bedzie nadal mogl majstrowac, wiec lepiej na 0
 float derivative = 0.0;
 float previousError = 0.0;
 float distance_point = 18.0;
 int servo_zero = 0;
 int t = 100;
 // #define PIN_LEFT_MOTOR_SPEED 5 // int PIN_LEFT_MOTOR_FORWARD = A0;
 // int PIN_LEFT_MOTOR_REVERSE = A1;
 // #define PIN_LEFT_ENCODER 2 // #define PIN_RIGHT_MOTOR_SPEED 6 // int PIN_RIGHT_MOTOR_FORWARD = A2;
 // int PIN_RIGHT_MOTOR_REVERSE = A3;
 // #define PIN_RIGHT_ENCODER 3 // #include <EEPROM.h> // struct KonfiguracjaSilnikow { // int PIN_LEFT_MOTOR_FORWARD;
 ...
 bool podczcasTestu = false;
 // ====================================================================================== #define SERIAL_BAUD_RATE 9600 String odpowiedzDoUzytkownika= "{DONE,";
...
float get_dist(int n){
  ...
  float adc=sum/n;//zbiera wartosc 100 razy i usrednienie 
  float distance_cm = 17569.7 * pow(adc, -1.2062);//przeliczenie na cm 
  return(distance_cm);  //sciaga odleglosc od czurnika 
}

 void PID(){ //to jest algorytm caly praktycznie, 
 ...
 // Serial.print(" : ");
 // Serial.print(proportional);
 //blad pomiedzy tym co ma a tym co powinno byc 
 // Serial.print(" : ");
 // Serial.println(output);
 //wysjcie z pida, powinien być w zakresie –10/10 Serial.print(distance);
  Serial.print(" : ");
  Serial.print(proportional);
  Serial.print(" : ");
  Serial.print(output);
...
void WykonajRamke(String ramka){ 
 ...
 //przetwarzana ramka: P4.00, I0.30, D8.30, C45, T100, B1, C17.00 
  ...
  // bool czyCB = false;
//   if (ramka.indexOf("C")>=0 && ramka.indexOf("B")>=0){
//     // todo c i b czyCB = true;
//  }
  ...
  while (end != -1){ 
    ...
    if (komenda.startsWith("P")) Funkcja_P(komenda.substring(1).toFloat());//P4.00, 
      ...
      if (wartosc == 2) { // STOP awaryjny awaryjnyStop = true;
        Funkcja_S(2);
        return;
      }
      else { 
...
void loop() { 
  ...
  if (podczcasTestu){ 
    if (Serial.available()){ 
      while(Serial.available() > 0) Serial.read();// czyszczenie resztek bajtów 
      cmd = Serial.readStringUntil('\n');
        if (cmd.indexOf("KP")>=0) kp = cmd.substring(cmd.indexOf("P")+2, cmd.indexOf(",")).toFloat();
        if (cmd.indexOf("KI")>=0) ki = cmd.substring(cmd.indexOf("I")+2, cmd.indexOf(",")).toFloat();
        if (cmd.indexOf("KD")>=0) kd = cmd.substring(cmd.indexOf("D")+2, cmd.indexOf(",")).toFloat();
 ...
 // --- co 0.2 s sprawdza czy testRunning na true i czy moze dalej wykonac --- 
  static unsigned long lastPID = 0;
  if (podczcasTestu && millis() - lastPID >= 200) { 
    lastPID = millis();
    distance = get_dist(10);
    PID();
  }
}
